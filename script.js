    ...
    if (teamGrid && modalOverlay && modalContent) {
        ...
        teamGrid.addEventListener('click', (e) => {
            ...
            if (socialLink && teamCard) {
                 ...
                 // Only handle LinkedIn link click
                 if (member && socialLink.classList.contains('linkedin-link')) {
                     window.open(member.linkedin_url, '_blank');
                 }
                 e.stopPropagation(); // Prevent the modal from opening if a social link was clicked
                 return;

            } else if (teamCard) { // Handle click on the card itself (but not the social links within)
                 const memberId = teamCard.dataset.memberId;
                 const member = teamMembers.find(m => m.id === memberId);
